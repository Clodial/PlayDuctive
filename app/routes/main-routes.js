...
router.get('/', function(req,res){
    ...
    if(!req.session.user || !req.session.stats){
	   res.render('index', { title: 'PlayDuctive', proj: null, stats: req.session.stats, user: req.session.user});
    }else{
    ...
    con.query('CALL createAccount(?,?,?)', [user, pass, email],
        function (err, result) {
            if (err) {
                ...
                console.log(req.session.user);
                req.session.user = user;
                res.render('login', {title: 'PlayDuctive', proj: null, stats: req.session.stats, success: "success", user: req.session.user});
            }
        }
    );
});
